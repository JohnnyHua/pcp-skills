...
> 补充：路线图回答“实现顺序”，而 [PCP 版本基线](./pcp-version-baseline.md) 回答“当前这一版先做什么”。
...
`pcp_intake` 的下一步不是继续做成“只会读取 handoff”的恢复工具，而是升级成统一项目接手入口。

当前已确定的方向：

1. 先建立项目理解，再看是否已有 PCP
2. 有 handoff 时优先利用 handoff，但仍要结合当前项目状态
3. 没 handoff 但有 PCP 时，先展示当前流程，再让用户决定是否沿用
4. 没 handoff 也没 PCP 时，只给项目概况和下一步建议，不自动进入 `plan`
5. `worklog / backlog / concern / changelog` 默认只做存在提示、作用说明和轻摘要，避免 token 无脑堆叠
6. intake 的终点是“等待用户决定下一步”，不是自动开工
